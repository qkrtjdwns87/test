<?
	...
	$submitUrl = '/manage/main_m/trendwrite';
	$trendCnt = 12;
?>
<? include $_SERVER["DOCUMENT_ROOT"]."/inc/adm/header.php"; ?>
	...
	<script type="text/javascript">
		...
		function deleteContent(type, no, order){
			var url = '/manage/main_m/trendcontentdelete/mmno/<?=$mmNum?>';
...
<div id="container">
	...
	<div id="content">
		...
		<div class="title">
			<h2>[New Trending]</h2>
			<div class="location">Home &gt; 시스템관리 &gt; 메인 관리 &gt; New Trending</div>
		</div>
		...
		<table class="write2">
			...
			<?
				...
				for($i=0; $i<$trendCnt; $i++)
				{
					$fi = $i * 2;
					$mmtrNum = $trendOrder = 0;
					$itemNum =$itemName = $itemCode = '';
					$shopNum = $shopName = $shopCode = '';
					$fileNum = $mobFileNum = '';
					$fileName = $mobFileName = $imgUrl = $mobImgUrl = '';
					if (isset($recordSet) && isset($recTrendSet))
					{
						$mmtrNum = $recTrendSet[$i]['NUM'];
						$trendOrder = $recTrendSet[$i]['TREND_ORDER'];
						$itemName = $recTrendSet[$i]['ITEM_NAME'];
						$itemCode = $recTrendSet[$i]['ITEM_CODE'];
						$itemNum = $recTrendSet[$i]['SHOPITEM_NUM'];
						$shopName = $recTrendSet[$i]['SHOP_NAME'];
						$shopCode = $recTrendSet[$i]['SHOP_CODE'];
						$shopNum = $recTrendSet[$i]['SHOP_NUM'];
					}
					
					$img = $imgUrl = '';
					if (isset($recTrendSet[$i]['FILE_INFO']))
					{
						$arrFile = explode('|', $recTrendSet[$i]['FILE_INFO']);
						if (count($arrFile) > 0)
						{
					...
					$trendOrder = ($trendOrder == 0) ? $i : $trendOrder;					
				...
				<tr>
					...
					<td>
						<dl class="dl_img">
							...
							<dd>
								<?if (!empty($itemName)){?>
								<a href="/manage/item_m/updateform/sno/<?=$shopNum?>/sino/<?=$itemNum?>" class="alink" target="_blank"><?=$itemName?></a> <span>(<?=$itemCode?>)</span><?if ($i > 0){?><a href="javascript:deleteContent('item', '<?=$mmtrNum?>', '<?=$i?>');" class="close"><img src="/images/adm/btn_close.gif" alt="닫기" /></a><?}?>
								<?}?>
							</dd>
							<dd>
								<?if (!empty($shopName)){?>
							...
							<dd>
								<input type="text" id="itemname_<?=$i?>" name="trendmn[<?=$i?>][itemname]" value="" class="inp_sty40" readonly/>
								<input type="hidden" id="itemno_<?=$i?>"  name="trendmn[<?=$i?>][itemno]" value="<?=$itemNum?>" class="inp_sty40" />
					...
					<td>
						<input type="text" id="trendorder_<?=$i?>" name="trendmn[<?=$i?>][order]" value="<?=$trendOrder?>" class="inp_sty60" />
						<input type="hidden" name="trendmn[<?=$i?>][num]" value="<?=$mmtrNum?>" />						
					</td>
				</tr>
		...
		<div class="btn_list">
			...
			<a href="javascript:sendTrendMain();" class="btn2">저장</a>
		</div>
