<?
	...
	$submitUrl = '/manage/main_m/newitemwrite';
?>
<? include $_SERVER["DOCUMENT_ROOT"]."/inc/adm/header.php"; ?>
	...
	<script type="text/javascript">
		...
		function deleteContent(type, no, order){
			var url = '/manage/main_m/newitemcontentdelete/mmno/<?=$mmNum?>';
...
<div id="container">
	...
	<div id="content">
		...
		<div class="title">
			<h2>[신상품]</h2>
			<div class="location">Home &gt; 시스템관리 &gt; 메인 관리 &gt; 신상품</div>
		</div>
		...
		<table class="write2">
			...
			<?
				...
				for($i=0; $i<$newItemCnt; $i++)
				{
					...
					if (isset($recordSet) && isset($newItemSet))
					{
						if (isset($newItemSet[$i]))
						{
							$mmtNum = $newItemSet[$i]['NUM'];
							$itemOrder = $newItemSet[$i]['NEWITEM_ORDER'];
							$itemName = $newItemSet[$i]['ITEM_NAME'];
							$itemCode = $newItemSet[$i]['ITEM_CODE'];
							$itemNum = $newItemSet[$i]['SHOPITEM_NUM'];
							$shopName = $newItemSet[$i]['SHOP_NAME'];
							$shopCode = $newItemSet[$i]['SHOP_CODE'];
							$shopNum = $newItemSet[$i]['SHOP_NUM'];							
						}
					}
					
					$img = $imgUrl = '';
					if (isset($newItemSet[$i]['FILE_INFO']))
					{
						$arrFile = explode('|', $newItemSet[$i]['FILE_INFO']);
						if (count($arrFile) > 0)
						{
				...
				<tr>
					...
					<td>
						<dl class="dl_img">
							...
							<dd>
								<input type="text" id="itemname_<?=$i?>" name="newmn[<?=$i?>][itemname]" value="" class="inp_sty40" readonly/>
								<input type="hidden" id="itemno_<?=$i?>"  name="newmn[<?=$i?>][itemno]" value="<?=$itemNum?>" class="inp_sty40" />
					...
					<td>
						<input type="text" id="neworder_<?=$i?>" name="newmn[<?=$i?>][order]" value="<?=$itemOrder?>" class="inp_sty60" />
						<input type="hidden" name="newmn[<?=$i?>][num]" value="<?=$mmtNum?>" />						
					</td>
				</tr>
		...
		<div class="btn_list">
			...
			<a href="javascript:sendNewItemMain();" class="btn2">저장</a>
		</div>
