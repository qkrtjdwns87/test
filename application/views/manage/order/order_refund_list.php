<?
	...
	$listUrl = '/manage/order_m/refundlist'.$addUrl;
...
<div id="container">
	<div id="content">
		...
		<div class="title">
			<h2>[환불관리]</h2>
			<div class="location">Home &gt; 주문관리 &gt; 환불관리</div>
		</div>
		...
		<?
			...
			{
				$col2 = $ordStatsToDaySet['STATE_5130'];
				$col3 = $ordStatsToDaySet['STATE_5135'];
				$col4 = $ordStatsToDaySet['STATE_5140'];
				$col5 = $ordStatsToDaySet['STATE_5150'];
				$col6 = $ordStatsToDaySet['STATE_5160'];
				$col7 = $ordStatsToDaySet['STATE_5180'];
			}			
			
			//일자를 무시한 통계인 경우
			/*
			if (isset($ordStatsToDaySet[0]['STATE_5130']))
			{
		...
		?>
		<table class="write2">
			<colgroup><!-- <col /> --><col /><col width="12.5%" /><col width="12.5%" /><col width="12.5%" /><col width="12.5%" /><col width="12.5%" /></colgroup>
			<thead>
				<tr>
					<!-- <th>오늘 신규 환불신청</th> -->
					<th>환불신청</th>
		...
		<table class="write2">
			...
			?>			
				...
				<?}?>
					...
					<td width="8%"><?=substr($rs['REFUND_DATE'], 0, 10)?></td>
					<td width="8%"><?=substr($rs['REFUND_END_DATE'], 0, 10)?></td>
				</tr>
			<?
					$tmpOrderNum = $rs['NUM'];
		...
		<div class="btn_list">
			...
			<a href="javascript:grpOrderChange('5165', '반품불가');" class="btn1 fl_r">반품불가 처리</a>
			<!-- PG연동이 필요한 부분<a href="javascript:grpOrderChange('5180', '반품완료');" class="btn1 fl_r">반품완료 처리</a> -->			
			<a href="javascript:grpOrderChange('5135', '환불불가');" class="btn1 fl_r">환불불가 처리</a>
			<!-- PG연동이 필요한 부분<a href="javascript:grpOrderChange('5150', '환불완료');" class="btn1 fl_r">환불완료 처리</a> -->			
		</div>
		...
		<div class="help_tip">
			<dl>
				...
				<dd>- 환불완료 처리를 하셔야 고객의 구매취소가 완료됩니다.</dd>
				<dd>- 환불불가된 주문 건은 주문상세정보 팝업에서 주문상태를 수정해 주십시오.</dd>
				<dd>- 환불승인 처리 후 카드결제취소, 휴대폰 결제 취소, 환불액 입금 처리를 완료하신 다음, 해당 주문 건에 대해 환불완료 처리해 주십시오.</dd>
				<dd>- 환불을 요청한 고객이 반품Item 택배 운송장번호를 입력하면 반품처리중 상태로 자동 변환됩니다.</dd>
			</dl>
		</div>
