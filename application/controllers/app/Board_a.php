 ...
 * @version:
 */
class Board_a extends CI_Controller {
	...
	public function _remap()
	{
		...
		{
			case 9110:
				$brdListView = 'app/notice/qna_list';
				break;
			case 9130:
				$brdListView = 'app/notice/faq_list';
				break;				
			default:
				$brdListView = 'app/notice/news_list';
				break;
		}
		
		$brdView = 'app/notice/board_view';
		$brdWriteView = 'app/notice/board_write';
		...
		switch($this->_uriMethod)
		{
			case 'list':
				...
				$this->_data = array_merge($this->_data, $this->_sendData);
				($this->_outformat == 'json') ? $this->getBoardDataListToJson() : $this->load->view($brdListView, $this->_data);
				break;
			case 'writeform':
				$this->getGroupCodeDataList();	
			...
			case 'grpdelete':
				...
				break;	
			case 'notiurgency';
				$this->getAppNoticeUrgencyRowData();
				$data = array_merge($this->_data, $this->_sendData);
				$this->load->view('app/notice/notice_view_urgency', $data);
				break;
	...
	{
		if ($this->_uLevelType == 'SHOP') $this->_sendData['uNum'] = get_cookie('usernum'); //$this->common->getSession('user_num');
	...
	private function getBoardDataListToJson()
	{
	...
	private function setBoardDataInsert()
	{
		...
		$insData = array(
			...
			'USER_ID' => $userInfo['USER_ID'], //$this->common->getSession('user_id'),				
			'USER_NAME' => $userInfo['USER_NAME'], //$this->common->getSession('user_name'),
			'USER_EMAIL' => $userInfo['USER_EMAIL'], //$this->common->getSession('user_email'),
	 ...
	 * 
	 */
	private function getAppNoticeUrgencyRowData()
	{
		$this->_data['appNoticeSet'] = $this->board_model->getAppNoticeUrgencyRowData();
	}
}
?>